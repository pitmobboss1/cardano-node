...
hprop_kes_period_info = H.integrationRetryWorkspace 2 "kes-period-info" $ \tempAbsBasePath' -> do
  ...
  --H.cat . Text.unpack $ Text.decodeLatin1 $ LBS.toStrict $ encodePretty poolState'
  ...
  UTxO utxo1 <- H.noteShowM $ H.jsonErrorFail $ J.fromJSON @(UTxO ConwayEra) utxo1Json
  ...
  UTxO utxo2 <- H.noteShowM $ H.jsonErrorFail $ J.fromJSON @(UTxO ConwayEra) utxo2Json
  ...
  H.lbsWriteFile (testSpoDir </> "configuration.yaml") yamlBs
  _runtime <- startNode (TmpAbsolutePath tempAbsPath') "test-spo" 3005
        [ "run"
        , "--config", testSpoDir </> "configuration.yaml"
        , "--topology", topologyFile
        , "--database-path", testSpoDir </> "db"
        , "--shelley-kes-key", testSpoKesVKey
        , "--shelley-vrf-key", testSpoKesVKey
  ...
  -- TODO: Create utxo era specfic json decode function


-- decodeEraUTxO :: CardanoEra era ->
