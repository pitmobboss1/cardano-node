...
import           Data.Aeson (FromJSON, eitherDecodeFileStrict', decode, encode, parseJSON)
  ...
  = do
    ...
    forM_ bArgs $ \bArg -> do
      ...
      case preExecutePlutusScript protocolParameters script (getScriptData apiData) apiData of
        Left err -> die $ "--> execution failed: " ++ show err
...
checkPlutusLoop (Just PlutusOn{..})
  = do
    ...
    protocolParameters <- readProtocolParametersOrDie
    putStrLn $ "--> Read protocolParameters: " ++ show protocolParameters
    ...
    let
      blockMaxOut b d =
        case plutusAutoScaleBlockfit (pparamsStepFraction d) ("factor for block execution steps: " ++ show d) script b (TargetTxsPerBlock 8) 1 of
          ...
          Left err              -> die $ "plutusAutoScaleBlockfit: " ++ show err
...
readProtocolParametersOrDie
  = do
    ...
    putStrLn $ "--> parametersFile: " ++ show parametersFile
    valueParams <- eitherDecodeFileStrict' parametersFile
    protoParams' <- eitherDecodeFileStrict' parametersFile
    case valueParams of
      Left err -> die err
      Right vp -> do
        putStrLn $ "--> valueParams: " ++ show vp
        case vp of
          Object m -> case "costModels" `KeyMap.lookup` m of
            Nothing  -> putStrLn "No cost models?"
            Just val -> do putStrLn $ "--> CostModels: " ++ show val
                           case val of
                               Object o -> do putStrLn $ "--> Object: " ++ show o
                                              mapM_ (\(k, v) -> putStrLn $ "--> Entry: " ++ show k ++ " : " ++ show v) $ KeyMap.toList o
                               Array  a -> putStrLn $ "--> Array: " ++ show a
                               _        -> putStrLn $ "--> Scalar: " ++ show val
                           putStrLn $ "--> Parse: " ++ show (parseEither parseJSON val :: Either String (Map.Map AnyPlutusScriptVersion [Integer]))
          _ -> putStrLn "valueParams not an Object?"
        let protoParams = parseEither parseJSON vp
        case protoParams of
          Left err -> die err
          Right pp -> do
            putStrLn $ "--> protoParams: " ++ show pp
            putStrLn $ "--> costModels: " ++ show (protocolParamCostModels pp)
            if protoParams /= protoParams'
              then putStrLn $ "parses differ!!! one-step protoParams: "
                                  ++ show protoParams'
              else pure ()
            pure pp
